...
const DecisionModal: React.FC<DecisionModalProps> = ({ problemName, onCommit, onBail }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div className="relative w-full max-w-sm bg-zinc-900 border border-amber-500/50 rounded-3xl p-6 shadow-2xl shadow-amber-500/20 animate-in zoom-in-95 duration-200">
      ...
      <h2 className="text-center text-white font-black text-lg mb-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          onClick={onCommit}
          className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-indigo-600/30 cursor-pointer"
        >
          <Lock className="w-6 h-6" />
          <span className="text-sm">Yes — Commit 🔒</span>
          <span className="text-[10px] font-normal text-indigo-200 text-center">Start 20-min deep-work timer</span>
        </button>
        <button
          onClick={onBail}
          className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-gradient-to-br from-rose-600 to-orange-600 hover:from-rose-500 hover:to-orange-500 text-white font-bold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-rose-600/30 cursor-pointer"
        >
          <Rocket className="w-6 h-6" />
          <span className="text-sm">No — Bail 🚀</span>
          <span className="text-[10px] font-normal text-rose-200 text-center">Smart skip + capture notes</span>
        </button>
      </div>
    </div>
  </div>
...
const TriageMode: React.FC = () => {
  ...
  return (
    <div className="bg-zinc-950 border border-amber-500/30 rounded-3xl p-5 sm:p-7 shadow-2xl shadow-amber-500/10 relative overflow-hidden mt-2">
      ...
      {phase === 'idle' && (
        <div className="space-y-4">
          ...
          <p className="text-zinc-600 text-xs">
            A <span className="text-amber-400 font-semibold">90-second micro-timer</span> will start immediately. At 90 s you decide: Commit (20-min deep dive) or Bail (capture notes → next problem).
          </p>
        </div>
      )}
        ...
        <div className="flex flex-col items-center gap-6 py-2">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center">
              <span className="text-5xl font-black font-mono tabular-nums text-amber-300 drop-shadow-lg">
      ...
      {phase === 'commit' && (
        <div className="flex flex-col items-center gap-5 py-2">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center">
              <span className="text-5xl font-black font-mono tabular-nums text-indigo-200 drop-shadow-lg">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-zinc-700 text-sm font-semibold transition-all cursor-pointer flex items-center gap-2"
          >
                ...
                <tbody>
                  {log.map(entry => (
                    <tr key={entry.id} className="border-b border-zinc-900 last:border-0">
                      <td className="py-2 pr-3 text-zinc-200 font-medium max-w-[10rem] truncate">
                        {entry.problemName}
                      </td>
                      ...
                      <td className="py-2 text-zinc-500 max-w-[12rem] truncate">
                        {entry.partialNotes || '—'}
                      </td>
                    </tr>
                  ))}
      ...
      <div className="mt-5 flex items-center gap-3 text-[10px] text-zinc-600 border-t border-zinc-800/60 pt-3">
